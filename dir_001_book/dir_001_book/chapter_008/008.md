...
### Visualization of Class as UML

- **UML** (Unified Modeling Language) is a standardized way to visualize the design of a system.
  ![h:500](lkjfsdgvbsr.png)

---

### Tip: Static Variable

**Example**: A class with a static variable to count the number of objects created.
...
- The `Counter` class has a static variable `count` to keep track of the number of objects created.
- The constructor increments `count` each time a new object is created.
...
- Java automatically manages memory through **Garbage Collection** because it does not require manual memory management.
- When nothing refers to an object anymore, Java can automatically remove it from memory.
...
- Setting `myCircle` to `null` removes its reference to the object, so Java can delete it from memory.
